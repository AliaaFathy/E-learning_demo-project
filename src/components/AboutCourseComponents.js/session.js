...
function Session({data}){
            ...
            <Stack >
                <Stack sx={{position:'relative'}} >
                    <Stack direction='row' alignItems={'center'}  >
                        ...
                        <Typography sx={{left:400,position:'absolute',fontWeight:400,fontSize:15,color:'#767676'}}>{sessionofType.expected_time} {sessionofType.expected_time_unit}</Typography>
    ...
    return(
        <>
            <Stack direction='row'>
                <img
                    src={type==='text'?readingIcon :videoIcon}
                    style={{width:24,height:24,marginTop:5,marginRight:2}}
                    alt='video-reading'/>
                <Typography
                    sx={{fontWeight:700,fontSize:20,color:'#343434'}}>
                    {data.length} {type==='text'?data.length>1?'Readings':'Reading':data.length>1?'Videos':'Video'}
                </Typography>
            </Stack>
            {renderedSession}
            <Box
